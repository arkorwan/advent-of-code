...
rules.each{|r|
	lhs,rhs = r.split("=>")
	qr, symr = rhs.strip.split
	ingredients = lhs.split(",").map{|l| 
		ql, syml = l.strip.split
		[syml, ql.to_i]
	}
	@units[symr] = qr.to_i
	g[symr] ||= [0, nil]
	g[symr][1] = ingredients

	ingredients.each{|x,_|
		g[x] ||= [0, nil]
		g[x][0] += 1
	}
...
def require_ores(g, n)
	gg = g.each.map{|k,v| [k,v.clone]}.to_h
	q = ["FUEL"]
	h = {"FUEL" => n}
	until h.keys == ["ORE"]
		x = q.pop
