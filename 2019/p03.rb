...
@intersect = [[b1[0], b2[0]].min, [b1[1], b2[1]].max, [b1[2], b2[2]].min, [b1[3], b2[3]].max]
@offset_x = @intersect[1]
@offset_y = @intersect[3]


def prepare()
	@field = Array.new(@intersect[0]-@intersect[1]+1).map{ Array.new(@intersect[2]-@intersect[3], 0)}
	@min_cross = 2**31
end


def sim_part1(wire, k)
	x = -@offset_x
	y = -@offset_y
	wire.each{|d, n|
		dx, dy = case d
		...
		n.times{
			...
			if x >= 0 && x < @field.size && y >= 0 && y < @field[0].size
				@field[x][y] |= k
				if @field[x][y] == 3
					dist = (x + @offset_x).abs + (y + @offset_y).abs 
					@min_cross = dist if dist < @min_cross
				end
			end
		}
	}
